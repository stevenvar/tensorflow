...
llvm::Value* GetBatchDimByName(llvm::IRBuilderBase* b, int64_t multiplier,
  ...
  if (!loadedValue) {
    ...
    llvm::FunctionCallee printf_func =
        function->getParent()->getOrInsertFunction("printf", printf_type);
    ...
    llvm::Value* function_name = entry_builder.CreateGlobalStringPtr(
        function->getName(), "bdim_function_name");
    llvm::Value* offset_value = llvm::ConstantInt::get(i64Type, off);
    entry_builder.CreateCall(
        printf_func, {format_str, function_name, run_options, offset_value,
                      loadedValue});
  }
  if (multiplier < 1) {
