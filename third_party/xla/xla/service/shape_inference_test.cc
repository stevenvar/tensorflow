...
TEST_F(ShapeInferenceTest, InferReshapeDegenerateCombine) {
  ...
  const Shape operand = ShapeUtil::MakeShape(F32, {1, 1},
                                             std::vector<bool>{false, true});
  const auto status =
...
TEST_F(ShapeInferenceTest, InferReshapeCombine) {
  ...
  //   | reshape
  // [<=60]
  const Shape operand = ShapeUtil::MakeShape(F32, {6, 10},
                                             std::vector<bool>{false, true});
  const auto status =
...
TEST_F(ShapeInferenceTest, InferDynamicBroadcast) {
  ...
  const Shape operand_shape =
      ShapeUtil::MakeShape(F32, {15}, std::vector<bool>{true});
  const absl::StatusOr<Shape> inferred_shape =
...
TEST_F(ShapeInferenceTest, ConditionalDynamic) {
  ...
  const Shape static_shape =
      ShapeUtil::MakeShape(S32, {4}, std::vector<bool>{false});
  const Shape dynamic_shape =
      ShapeUtil::MakeShape(S32, {4}, std::vector<bool>{true});
  const absl::StatusOr<Shape> inferred_shape0 =
      ShapeInference::InferConditionalShape(
          r0s32,
